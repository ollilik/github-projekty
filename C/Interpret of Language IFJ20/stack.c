...
*/
	if (s == NULL) stackError (SERR_INIT); //fce dostala spatne parametry vola fci stackError
	else s->top = -1; //nastaveni vrcholu zasobniku na hodnotu -1
}
...
*/
	return (s->top == -1) ? 1 : 0; //uziti terarniho operatoru pro navrat hodnoty 1 - prazdny zasobnik 0 - zasobnik neni prazdny
}
...
*/
	return (s->top < STACK_SIZE - 1) ? 0 : 1; //uziti terarniho operatoru pro navrat hodnoty 1 - plny zasobnik 0 - zasobnik neni plny
}
...
** funkci stackEmpty.
*/
	if (stackEmpty (s)==1) return -2; //pokud je zasobnik prazdny vola se fce stackError
	else  return s->arr[s->top]; //ukazatel prijaty v paramatru c ukazuje na vrchol zasobniku
}
...
**
*/
	if (stackEmpty (s)==0) s->top--; //pokud neni zasbonik prazdny, dekrementuje se ukazatel vrcholu zasobniku
}
...
** funkci stackFull.
*/
	if (stackFull(s)!=0) { //pokud je zasobnik prazdny vola se fce stackError
		stackError(SERR_PUSH);
	}
	else { //inkrementace ukazatele vrcholu zasobniku a pridani prvni na vrchol
		s->top++;
		s->arr[s->top] = i;
	}
}
