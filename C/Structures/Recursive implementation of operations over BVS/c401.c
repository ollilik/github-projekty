...
**/
	...
	else if(RootPtr->Key > K) return BSTSearch(RootPtr->LPtr,K,Content);
...
**/
	...
	if(*RootPtr == NULL)
	{
		...
		if(tmp == NULL) return; // chyba pri malloc
		tmp->Key = K;
		tmp->LPtr = NULL;
		tmp->RPtr = NULL;
	...
	else if((*RootPtr)->Key == K) (*RootPtr)->BSTNodeCont = Content;
	else if((*RootPtr)->Key < K) BSTInsert(&(*RootPtr)->RPtr,K,Content);
	else BSTInsert(&(*RootPtr)->LPtr,K,Content);
