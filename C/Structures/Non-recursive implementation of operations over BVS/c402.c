...
**/
	...
	tBTNodePtr tmp_search = *RootPtr;
	bool not_fnd = true;
	while(not_fnd)
	{
		if (tmp_search == NULL) break;
		else if(Content > tmp_search->Cont && tmp_search->RPtr != NULL)
			tmp_search = tmp_search->RPtr;
		else if(Content < tmp_search->Cont && tmp_search->LPtr != NULL)
			tmp_search = tmp_search->LPtr;
		else if(Content == tmp_search->Cont)
			not_fnd = false;
		else
			break;
	}
	if(not_fnd)
	{
		tBTNodePtr tmp_new = malloc(sizeof(struct tBTNode));
		if(tmp_new == NULL) return;
		tmp_new->RPtr = NULL;
		tmp_new->LPtr = NULL;
		tmp_new->Cont = Content;
		if(tmp_search == NULL) (*RootPtr) = tmp_new;
		else if (Content < tmp_search->Cont) tmp_search->LPtr = tmp_new;
		else tmp_search->RPtr = tmp_new;
	}
...
**/
	...
	while(!SEmptyP(&stackP)) Leftmost_Preorder(STopPopP(&stackP)->RPtr,&stackP);
...
**/
	...
	while((*RootPtr) != NULL)
	{
		if((*RootPtr)->RPtr) SPushP(&stackP, (*RootPtr)->RPtr);
